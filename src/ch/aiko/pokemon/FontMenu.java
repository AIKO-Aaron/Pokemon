...
public class FontMenu extends Menu {
	...
	public void paint(Drawer d) {
		...
		for (int i = 0; i < user.length; i++) {
			d.drawText(user[i].getFontName() + (getFileName(user[index - fonts.length].getFontName().replace(" ", "-")).equalsIgnoreCase(Settings.font) ? " (!) " : "") + (i == index - fonts.length ? "  <---" : ""), 50, (i + fonts.length - y + 4) * 25, 25, 0xFFFFFFFF, user[i].getFontName());
		}
	}
