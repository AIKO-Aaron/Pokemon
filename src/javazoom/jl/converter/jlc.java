...
/**
 * The <code>jlc</code> class presents the JavaLayer Conversion functionality as a command-line program.
 *
 * @since 0.0.7
 */
public class jlc {
	...
	static public void main(String args[]) {
		...
		if (!ma.processArgs(argv)) System.exit(1);
	...
	static class jlcArgs {
		...
		public boolean processArgs(String[] argv) {
			...
			output_filename = "";
			if (argc < 2 || argv[1].equals("-h")) return Usage();
			...
			while (i < argc) {
				...
				if (argv[i].charAt(0) == '-') {
					...
					/*
					 * else if (argv[i].equals("-s")) ma.stdout_mode = true;
					 */
					...
					/*
					 * else if (argv[i].equals("-f")) { if (++i == argc) { System.out.println("Please specify a new scalefactor after the -f option!"); System.exit(1); } ma.use_own_scalefactor = true; // ma.scalefactor = argv[i]; }
					 */
					else return Usage();
				} else {
					filename = argv[i];
